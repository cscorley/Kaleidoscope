...
void dumpKeyLEDState(const Simulator &simulator) {
   ...
   for(uint8_t row = 0; row < rows; ++row) {
      for(uint8_t col = 0; col < cols; ++col) {
         ...
         simulator_core.getCurrentKeyLEDColor(row, col, 
                               red, green, blue);
                       ...
                       const uint8_t key_led_colors[][3])
{
   ...
   for(uint8_t row = 0; row < rows; ++row) {
      for(uint8_t col = 0; col < cols; ++col) {
         ...
         const uint8_t &red_expected = key_led_colors[pos][0];
         const uint8_t &green_expected = key_led_colors[pos][1];
         const uint8_t &blue_expected = key_led_colors[pos][2];
